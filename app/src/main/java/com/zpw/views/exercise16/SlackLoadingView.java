package com.zpw.views.exercise16;
...
/**
 * Created by zpw on 2018/7/4.
 */

public class SlackLoadingView extends View {
    private int[] mColors = new int[]{0xB07ECBDA, 0xB0E6A92C, 0xB0D6014D, 0xB05ABA94};//四条杠的颜色
    private int mWidth, mHeight;//控件宽高
    private final int CANVAS_ROTATE_ANGLE = 60;//Canvas起始旋转角度
    private int mCanvasAngle;//Canvas旋转角度
    private final int MIN_LINE_LENGTH = dp2px(40);//线条最短长度
    private final int MAX_LINE_LENGTH = dp2px(120);//线条最大长度
    private int mEntireLineLength = MIN_LINE_LENGTH;//线条总长度
    private float mLineLength;//线条长度
    private int mCircleRadius;//圆半径
    private Paint mPaint;
    private int mStep;//第几步动画

    private final int STATUS_STILL = 0;//静止状态
    private final int STATUS_LOADING = 1;//加载状态

    private int mStatus = STATUS_STILL;//动画当前状态
    private List<Animator> mAnimList = new ArrayList<>();//所有动画
    private final int MIN_DURATION = 500;//最小间隔时长
    private final int MAX_DURATION = 3000;//最大间隔时长
    private int mDuration = MAX_DURATION;//动画间隔时长

    private float mCircleY;//半圆Y轴位置

    public SlackLoadingView(Context context) {
        this(context, null);
    }

    public SlackLoadingView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        if (isInEditMode()) {
            return;
        }
    ...
    private void startCRAnim() {
        ValueAnimator canvasRotateAnim = ValueAnimator.ofInt(mCanvasAngle + 0, mCanvasAngle + 180);
    ...
    private void startLCAnim() {
        ValueAnimator lineWidthAnim = ValueAnimator.ofFloat(mEntireLineLength - dp2px(1), -mEntireLineLength);
