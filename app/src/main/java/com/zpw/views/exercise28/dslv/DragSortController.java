package com.zpw.views.exercise28.dslv;
...
public class DragSortController extends SimpleFloatViewManager implements View.OnTouchListener, GestureDetector.OnGestureListener {
    ...
    public boolean onTouch(View v, MotionEvent ev) {
        ...
        //将触摸事件传递给自定义手势拦截器处理。onDown，onScroll，onSingleTapUp，onFling 会在 onTouchEvent 分别回调。
        mDetector.onTouchEvent(ev);

        //将触摸事件传递给自定义fling手势拦截器处理。onDown，onScroll，onSingleTapUp，onFling 会在 onTouchEvent 分别回调。
        ...
        return false;//返回false代表不消耗触摸事件，将会调用 listview 的 onTouchEvent 方法。
    }
    ...
    public boolean onDown(MotionEvent ev) {
        ...
        mHitPos = startDragPosition(ev);//点击的位置
